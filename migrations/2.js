...
async function up({ db, progress }) {
  ...
  const legacyShopPermissionMap = {
    ...
    "create-product": ["reaction:legacy:navigationTrees-drafts/read"],
    createProduct: [
    ...
    dashboard: ["reaction:legacy:emails/read"],
    "discounts/apply": ["reaction:legacy:carts:/update"],
    "media/create": ["reaction:legacy:mediaRecords/create:media"],
    "media/delete": ["reaction:legacy:mediaRecords/delete:media"],
    "media/update": [
    ...
    "order/view": ["reaction:legacy:orders/read"],
    "product/admin": [
      "reaction:legacy:products/update:prices",
      "reaction:legacy:products/read",
      "reaction:legacy:products/update",
    ...
    "product/archive": ["reaction:legacy:products/archive"],
    "product/clone": ["reaction:legacy:products/clone"],
    "product/create": ["reaction:legacy:products/create"],
    "product/publish": ["reaction:legacy:products/publish"],
    "product/update": [
      "reaction:legacy:products/update:prices",
      "reaction:legacy:products/update",
    ],
    "reaction-email": ["reaction:legacy:emails/send"],
    "reaction-templates": ["reaction:legacy:email-templates/update"],
    shipping: [
    ...
    tags: ["reaction:legacy:tags-inactive/read"],
    "tags/admin": ["reaction:legacy:tags/read", "reaction:legacy:tags/update"],
    "tags/edit": ["reaction:legacy:tags/read", "reaction:legacy:tags/update"],
    "taxes/read": ["reaction:legacy:taxes/read"],
    "taxes/write": ["reaction:legacy:taxes/read"],
    owner: [
  ...
  if (groups && Array.isArray(groups)) {
    ...
    for (let index = 0; index < groups.length; index += 1) {
      ...
      const finalPermissions = uniqueNewPermissions.filter(
        (permission) =>
          !legacyShopPermissions.includes(permission) && permission !== null
      );
  ...
  if (!groups.find((group) => group.slug === "sub-admin")) {
    ...
    newGroups.push({
      ...
      name: "sub-admin",
      slug: "sub-admin",
      permissions: ["reaction:legacy:quote/read"],
      shopId: null,
    });
  }
  ...
  if (newGroups.length) {
    ...
    const customerGroup =
      groups.find((group) => group.slug === "customer") || {};
    const accounts = await db
      .collection("Accounts")
      .find({ groups: { $nin: [customerGroup._id] } })
      .toArray();
    ...
    for (let index = 0; index < accounts.length; index += 1) {
      ...
      if (
        ...
        user.roles.__global_roles__.includes("owner") &&
        systemManagerGroupId
      ) {
        accountGroups.push(systemManagerGroupId);
      } else if (
        user.roles.__global_roles__ &&
        user.roles.__global_roles__.includes("admin") &&
        accountsManagerGroupId
      ) {
        accountGroups.push(accountsManagerGroupId);
      } else if (accountsManagerGroupId) {
        const primaryShop = db
          .collection("Shops")
          .findOne({ shopType: "primary" });
        if (
          user.roles[primaryShop._id] &&
          user.roles[primaryShop._id].find((perm) =>
            ["admin", "owner"].includes(perm)
          )
        ) {
          accountGroups.push(accountsManagerGroupId);
        }
