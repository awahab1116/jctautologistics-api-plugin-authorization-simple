...
export default async function preStartup(context) {
  console.log("in authorization simple");
  ...
  if (!ok) {
    throw new Error(
      `Database needs migrating. The "${migrationsNamespace}" namespace must be at version ${expectedVersion}. See docs for more information on migrations: https://github.com/reactioncommerce/api-migrations`
    );
  }
}
